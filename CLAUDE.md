...
- **JetSeeker** — flight inventory + PNR/ticket issuance (backend plumbing for MVP). Stack: Symfony/PHP + PostgreSQL/Doctrine + Amadeus (SOAP) + HubSpot CRM, on GitLab. Per Sprint-8 gating decision Q1 (locked 2026-04-25): flight purchase is fully embedded in Koncie — guests select suggested flights inside the Koncie UI and pay through the Koncie cart with Kovena MoR. There is no JetSeeker-branded customer-facing checkout. JetSeeker is reduced to backend inventory + PNR/ticket issuance behind Koncie's embedded flight surface. Recommended MVP integration: thin Symfony provider API on JetSeeker that Koncie calls for flight suggestions (origin + dates → carrier offers) and post-checkout to commit a PNR; plus a Symfony webhook for downstream time-change/cancellation events. Do not read JetSeeker's database directly. Per Q2 (locked 2026-04-25): suggested flights show carrier-level branding (Jetstar / Fiji Airways / Virgin / etc.) with a small "Sourced via JetSeeker" provenance footer in the modal.
